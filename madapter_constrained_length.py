# Main implementation of M-Adapter (constrained prompt length)
...
def get_kl_div_sims(args, test_features, val_features, train_features, clip_weights):
    ...
    return test_kl_divs_sim, val_kl_divs_sim
...
if __name__ == '__main__':
    ...
    for model_ in req_models:
        ...
        if args.sus_type == 'caps':
            support_features_path = os.path.join(features_path, str(args.prompt_length), 'caps_{}_f_m{}.pt'.format(dataset, disp_name))
            support_labels_path = os.path.join(features_path, str(args.prompt_length), 'caps_{}_t_m{}.pt'.format(dataset, disp_name))

            support_caption_features_path = os.path.join(features_path, str(args.prompt_length), 'caps_{}_c_m{}.pt'.format(dataset, disp_name))
        ...
        test_kl_divs_sims, val_kl_divs_sims = get_kl_div_sims(args, test_features, val_features, support_features, text_classifier_weights)
        ...
        logging.info('Best for Dataset: {}, Model: {}, SuS Type: {}, k: {}, alpha: {}, beta: {}, gamma: {}, delta: {}, CapS-Adapter Accuracy: {}, Prompt length: {}'.format(args.dataset, args.backbone, args.sus_type, args.k, best_alpha_tipx, best_beta_tipx, best_gamma_tipx, best_delta_tipx, tipx_acc, args.prompt_length))
    print('----------------------------------------------------------------------------')
    logging.info('----------------------------------------------------------------------------')
